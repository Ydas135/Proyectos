document.addEventListener("DOMContentLoaded", () => {
  ...
  function createHeart(){
    hearts.push({
      ...
      rotationSpeed: (Math.random() - 0.5) * 0.05, // velocidad de giro
      scale: 1, // tamaño inicial
      scaleSpeed: (Math.random() - 0.5) * 0.02 // pulso de escala
    });
  }
  ...
  function drawHearts(){
    ...
    hearts.forEach((h, i) => {
      ...
      ctx.scale(h.scale, h.scale); // efecto de agrandar/achicar
      ctx.fillStyle = h.color;

      ctx.beginPath();
      ctx.moveTo(0, 0);
      ctx.bezierCurveTo(-h.size, -h.size, -h.size * 2, h.size / 3, 0, h.size);
      ctx.bezierCurveTo(h.size * 2, h.size / 3, h.size, -h.size, 0, 0);
      ctx.fill();
      ...
      h.scale += h.scaleSpeed;

      // limitar el tamaño para que no desaparezca o se deforme demasiado
      if (h.scale < 0.8 || h.scale > 1.2) {
        h.scaleSpeed *= -1; // invertir dirección del pulso
      }
